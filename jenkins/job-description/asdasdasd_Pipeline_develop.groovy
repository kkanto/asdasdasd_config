...
def job_config = [
        job: [
                name: "asdasdasd_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1669378179574"
        ]
]
...
def customLib = library identifier: 'asdasdasd_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/asdasdasd_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.asdasdasd.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "asdasdasd", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("asdasdasd_Pipeline_develop");

job.with {

    authenticationToken('ApvUnYbRizEeBqwLUzqwt6VVhqvNZeUE')
